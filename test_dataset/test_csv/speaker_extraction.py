...
def create_patient_files(path: str | os.PathLike = None) -> None:
    """
    params:
        path - path to directory containing .csv files (default is current directory this file is located in)
    Iterates through the .csv files in the directory and creates a .txt file with only the patient's sentences
    within the patient_txt_files directory one level above the path directory
    """
    if path:
        os.chdir(path)
    ...
    for file in file_names:
        df = speaker_extraction(file, 2)
        sentence_df = sentence_extraction(df)
        string = concat_to_str(sentence_df)
        write_files(string, file.replace('.csv', '.txt'), '../patient_txt_files')

def create_qa_files(path: str | os.PathLike = None) -> None:
    """
    params:
        path - path to directory containing .csv files (default is current directory this file is located in)
    Iterates through the .csv files in the directory and creates a .txt file with the question and answer pairs
    within the question_and_answer_txt_files directory one level above the path directory
    """
    if path:
        os.chdir(path)
    file_names = os.listdir()
...
if __name__ == '__main__':
    # Creating patient txt files for test folder
